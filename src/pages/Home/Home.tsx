...
const Home= () => {
  ...
  //funcion de peticion get a api
  const fetchMovies = async (searchKey, pageNumber) => {
    const type = searchKey ? "search" : "discover";
    const {
      data: { results, total_pages },
    } = await axios.get(`${API_URL}/${type}/movie?${LANG}&page=${pageNumber}`, {
      params: {
        api_key: API_KEY,
        query: searchKey,
      },
            ...
            <div key={movie.id} className="col">
              ...
              <div className="card">
                ...
                <div className="card-body">
                  ...
                  <p className="card-genre"> {movie.release_date !== undefined ? movie.release_date.substring(0, 4): ""}
                  {" · "}
                  ...
                  <p className="card-rating">
                    <Rating name="half-rating-read" defaultValue={(movie.vote_average / 2).toFixed(1)} precision={0.5} readOnly 
                    icon={<Star style={{ color: 'white' }} />} emptyIcon={<StarBorder style={{ color: 'white' }} />}
                    />
                  </p>
                </div>
              </div>
